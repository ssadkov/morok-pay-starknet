...
export async function bisect(probe, start, end, tracker, lengthOnly) {
    if (start >= end)
        return; // base case: nothing to search
    const tr = tracker ?? new Tracker();
...
export async function scan(probe, start, tracker, lengthOnly) {
    const tr = tracker ?? new Tracker();
