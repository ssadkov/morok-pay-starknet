...
import { compute_enc_channel_key_hash, compute_enc_sender_addr_hash, compute_enc_token_hash, compute_enc_amount_hash, compute_enc_recipient_addr_hash, compute_enc_private_key_hash, compute_enc_user_addr_hash, } from "./hashes.js";
...
export const encryptions = {
    ...
    decryptChannelInfo: (encrypted, recipientPrivateKey) => {
        ...
        const key = (((toBigInt(encrypted.enc_channel_key) - compute_enc_channel_key_hash(sharedX)) %
            FIELD_PRIME) +
            FIELD_PRIME) %
            FIELD_PRIME;
        const sender = (((toBigInt(encrypted.enc_sender_addr) - compute_enc_sender_addr_hash(sharedX)) %
            FIELD_PRIME) +
            FIELD_PRIME) %
            FIELD_PRIME;
        return { key, sender };
    ...
    decryptPrivateKey: (encrypted, auditorPrivateKey) => {
        ...
        const sharedX = getXCoordinateFromBytes(sharedPoint);
        // Decrypt using field subtraction (matching Cairo)
        const privateKey = (((encrypted.encPrivateKey - compute_enc_private_key_hash(sharedX)) % FIELD_PRIME) +
            FIELD_PRIME) %
            FIELD_PRIME;
        return privateKey;
    },
    ...
    decryptUserAddr: (encrypted, auditorPrivateKey) => {
        ...
        const sharedX = getXCoordinateFromBytes(sharedPoint);
        // Decrypt using field subtraction (matching Cairo)
        const userAddr = (((encrypted.encUserAddr - compute_enc_user_addr_hash(sharedX)) % FIELD_PRIME) +
            FIELD_PRIME) %
            FIELD_PRIME;
        return userAddr;
    },
