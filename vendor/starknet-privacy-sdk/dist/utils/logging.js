 ...
 */
export function withLogging(target, name, callback) {
        ...
        get(obj, prop, receiver) {
            ...
            if (typeof value === "function" && typeof prop === "string" && !prop.startsWith("_")) {
                return function (...args) {
                    ...
                    const context = { id: traceId, childCounter: 0 };
...
export const isDebugEnabled = (targetName) => {
...
const isDebugEnabledForTarget = (name) => {
    const env = getEnv(DEBUG_ENV_VAR);
    if (!env)
        return false;
    if (env === "1" || env === "true")
 ...
 */
export const consoleLogCallback = (targetName, methodName, args, result, phase, traceId) => {
    // Check if debug is enabled for the specific method path
    if (!isDebugEnabled(`${targetName}.${methodName}`))
        return;
    const format = (value) => {
 ...
 */
export const debugLog = (target, sub, ...args) => {
