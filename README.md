...
**Pay privately** from a shielded Ready balance, or **sell a product** with a fixed-price QR. Top up from Base through CCTP when the private wallet is empty.

## Product

1. Connect Ready (Wallet API v6). Ready holds the STRK20 viewing key and talks to the official proving service.
2. **Sell** — enter a product and price, create a private checkout QR, then track `Awaiting payment → Paid → Fulfilled` locally.
...
The reference lives in the payment request; it is not written into the private transfer. Ready does not expose private history to dapps (`wallet_strk20Balances` only), so the sidebar lists activity this browser recorded. The merchant refreshes their private balance, confirms the matching sale, and separately marks the order fulfilled. Why the deployed invoice event is not trusted as payment proof is documented in [docs/private-invoices.md](docs/private-invoices.md).
...
2. **Merchant demo.** Sell a coffee on Sepolia from a second Ready, refresh the seller balance, confirm the sale, mark it fulfilled, and show the persistent product and revenue summary. Donation contests and cryptographic invoices stay outside this first cut.
...
Mainnet payments use the normal STRK20 transfer. The till reconciles with a private-balance refresh and `Confirm paid`; the event helper cannot bind an event to the transfer's hidden recipient and amount.
...
The legacy claim-link contract and `/claim` receiver remain experimental. Claim-link creation is not part of the focused Sale interface.
